...
module.exports = function normalizeDatastoreConfig (dsConfig, whitelist, expectedProtocolPrefix) {
  ...
  // Otherwise, normalize & parse the connection URL.
    ...
    // dsconfig -- unless a `protocol` property is already defined.
    if (parsedConnectionStr !== 'db:' && dsConfig.protocol === undefined) {
      dsConfig.protocol = parsedConnectionStr.protocol;
    }
