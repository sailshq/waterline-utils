...
module.exports = function planner(options) {
  ...
  if (_.isUndefined(options.criteria) || !_.isPlainObject(options.criteria)) {
    throw new Error('Options must contain a criteria dictionary.');
  }
  ...
  var groupedAssociations = _.groupBy(criteria.joins, 'alias');
  ...
  var determineStrategy = function determineStrategy(instructions) {
    ...
    if (!parentIdentity) {
      throw new Error('Unable to find a parentIdentity in ' + util.inspect(instructions, false, null));
    }

    if (!childIdentity) {
      throw new Error('Unable to find a childIdentity in ' + util.inspect(instructions, false, null));
    }

    // Calculate the parent and child primary keys
    var parentPk;
    try {
      parentPk = getPk(parentIdentity);
    } catch (e) {
      throw new Error('Error finding a primary key attribute for ' + parentIdentity + '\n\n' + e.stack);
    }
    ...
    if (strategy === strategies.VIA_JUNCTOR) {
      meta.junctorIdentity = _.first(instructions).child;
