...
module.exports = function convertCriteria(options) {
  ...
  _.each(instructions, function processJoins(val, key) {
    ...
    _.each(val.instructions, function checkForCriteria(joinSet) {
      ...
      // Check for SLOW JOIN criteria keys (skip and limit) and make sure if they
      // are set that they aren't the "base" values set by Waterline.
      ...
      if (_.has(joinSet.criteria, 'limit') || _.has(joinSet.criteria, 'skip')) {
        slowJoin = true;
        slowJoinAliases.push(key);
      }
    });
  ...
  _.each(_.uniq(slowJoinAliases), function buildJoinTemplate(alias) {
    ...
    if (strategy === 2 && !paginated) {
      (function generateTemplate() {
        ...
        whereTemplate[parentInstructions.child + '.' + parentInstructions.childKey] = {
          in: []
        };
    ...
    if (strategy === 3 && childPaginated) {
      (function generateTemplate() {
        ...
        var modifiedInstructions =  _.merge({}, childInstructions.criteria.instructions);
        modifiedInstructions[alias].instructions = [childInstructions];
