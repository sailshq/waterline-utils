...
/**
 * Write log messages to a log file. Fallback to console.log if the environment is browser.
 *
 * @param  {String}   absPathToLogFile
 * @param  {String}   logFileContents
 * @param  {Function} done
 */
var writeLogContents = function (absPathToLogFile, logFileContents, done) {

  // Write logs to console if environment is browser
  if (typeof window === 'undefined') {
    console.log(logFileContents);
    return done();
  }

  // Make sure we don't require('fs-extra') for non browser environments
  var fsx = require('fs-extra');

  // (see https://github.com/jprichardson/node-fs-extra/tree/0.30.0#outputfilefile-data-options-callback)
  fsx.outputFile(absPathToLogFile, logFileContents, done);
}
...
module.exports = function informReFailedAlterStratagem(err, operationName, modelIdentity, backupRecords, done) {
  ...
  writeLogContents(absPathToLogFile, logFileContents, function (err) {
    if (err) {
      console.error('\n'+
