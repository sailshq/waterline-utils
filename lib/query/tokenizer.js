...
var processFrom = function processFrom(value, results) {
  ...
  if (_.isObject(value)) {
    // Check if object contains an operator
    var key        = _.keys(value)[0];
    var identifier = identifiers[key];

    if (identifier && identifier === 'OPERATOR') {
      // If so, process as operator
      results.push({
        type:  'KEY',
        value: 'from'
      });

      processOperator(key, value[key], results);

      return;
    } else if (!_.isFunction(value) && !_.isArray(value)) {
      // Otherwise process as FROM clause
