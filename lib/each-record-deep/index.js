  ...
  _.each(records, function iterateRecords(record) {
    ...
    _.each(ParentWLModel.attributes, function iterateAttributes(attrDef, attrName) {
      ...
      if (attrDef.model) {
        ...
        if (_.isObject(childRecord) && !_.isArray(childRecord) && !_.isFunction(childRecord) && !_.isUndefined(childRecord[childPkProp]) && mapOfProcessedRecords[childModelIdentity][childRecord[childPkProp]]) {
          return;
        }
        ...
        _.each(childRecords, function iterateChildRecords(thisChildRecord) {
          // If we've already processed this child record, just return.
          if (_.isObject(thisChildRecord) && !_.isArray(thisChildRecord) && !_.isFunction(thisChildRecord) && !_.isUndefined(thisChildRecord[childPkProp]) && mapOfProcessedRecords[childModelIdentity][thisChildRecord[childPkProp]]) {
            return;
          }
          // Otherwise indicate that this child record is being processed so we don't do it again.
