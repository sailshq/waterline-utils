...
module.exports = function eachRecordDeep(records, iteratee, arePhysical, modelIdentity, orm) {
  ...
  // Create a dictionary to hold references to child records that have already been processed,
  // so that we don't process them again (e.g. to avoid JSON.parse errors for `json` attributes).
  // The map will be "childModelIdentity" => "dict of child ID => true"
  var mapOfProcessedRecords = {};
  ...
  _.each(records, function iterateRecords(record) {
    ...
    _.each(ParentWLModel.attributes, function iterateAttributes(attrDef, attrName) {
      ...
      // Get the property in the child record that represents its primary key.
      var childPkProp;
      if (arePhysical) {
        childPkProp = ChildWLModel.attributes[ChildWLModel.primaryKey].columnName;
      }
      else {
        childPkProp = ChildWLModel.primaryKey;
      }
      ...
      if (attrDef.model) {
        ...
        // Ensure that we have a mapping dictionary for this child model class.
        mapOfProcessedRecords[childModelIdentity] = mapOfProcessedRecords[childModelIdentity] || {};

        // If we've already processed this child record, just return.
        if (childRecord[childPkProp] && mapOfProcessedRecords[childModelIdentity][childRecord[childPkProp]]) {
          return;
        }
        // Otherwise indicate that this child record is being processed so we don't do it again.
        mapOfProcessedRecords[childModelIdentity][childRecord[childPkProp]] = true;
        ...
        _.each(childRecords, function iterateChildRecords(thisChildRecord) {
          ...
          // Determine if the child record is a dictionary.
          var isDictionary = _.isObject(thisChildRecord) && !_.isArray(thisChildRecord) && !_.isFunction(thisChildRecord);

          // If the child record is a dictionary...
          if (isDictionary) {
            // If we've already processed the record once, just continue.
            if (thisChildRecord[childPkProp] && mapOfProcessedRecords[childModelIdentity][thisChildRecord[childPkProp]]) {
              return;
            }
            // Otherwise indicate that this child record is being processed so we don't do it again.
            mapOfProcessedRecords[childModelIdentity][thisChildRecord[childPkProp]] = true;
          }
